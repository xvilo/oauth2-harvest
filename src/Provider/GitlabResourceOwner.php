...
class harvestResourceOwner implements ResourceOwnerInterface
{
     ...
     *
     * Requires optional harvest API client to be installed.
     *
     * @return Client
     */
    public function getApiClient()
    {
        if (!class_exists('\\harvest\\Client')) {
            throw new \LogicException(__METHOD__ . ' requires package m4tthumphrey/php-harvest-api to be installed and autoloaded'); // @codeCoverageIgnore
        }
        $client = \harvest\Client::create(rtrim($this->domain, '/') . self::PATH_API);
